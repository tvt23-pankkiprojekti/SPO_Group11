...
test("create a card_account entry", async () => {
    const result = await fetch(`http://localhost:${config.PORT}/api/card_account`, {
        method: "POST",
        body: JSON.stringify({
            Card_id: 1,
            Account_id: 1
...
test("get a card_account entry", async () => {
    const result = await fetch(`http://localhost:${config.PORT}/api/card_account/1`);
    expect(result.status).toEqual(200);
});

test("get a card_account entry that does not exist", async () => {
    const result = await fetch(`http://localhost:${config.PORT}/api/card_account/0`);
    expect(result.status).toEqual(404);
});

test("update a card_account entry", async () => {
    const result = await fetch(`http://localhost:${config.PORT}/api/card_account/11`, {
        method: "PUT",
        body: JSON.stringify({
            Card_id: 2,
            Account_id: 2
